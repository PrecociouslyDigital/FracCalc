	...
	public Fraction(String input) throws NotAFractionException{
		...
		if(slash != -1){/*
			...
			if(input.indexOf('_') != -1){
				this.whole = Integer.parseInt(input.split("_", 1)[0]);
				hasWhole = true;
			}else{
				this.whole = 0;
			}
			york = input.split("/", 1)[0];
			if(hasWhole)
				york = york.split("_", 1)[1];
			wyoming = input.split("/", 1)[1];
			...
			*/
			...
			if(underscore == -1){
				...
				this.denominator = Integer.parseInt(splitted[1]);
                           ...
                           /* System.out.println(input.substring(0, underscore-1));
                            System.out.println(input.substring(underscore+1,slash-1));
                                    System.out.println(input.substring(slash+1));*/
